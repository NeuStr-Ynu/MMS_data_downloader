...
def themis_download_bs4_mt(file_prefix, tint, destination_dir, opts=None, log_func=None, progress_func=None):
    """
    THEMIS downloader mirroring the SPDF directory structure.

    Parameters
    ----------
    file_prefix : str
        e.g. 'tha_l2_fgm'  ->  URL: themis/tha/l2/fgm/YYYY/MM/
    tint : tuple
        (start, end), either UTC datetimes or unix timestamps
    destination_dir : str | Path
    opts : dict
    ...
    var_parts = file_prefix.split("_")
    var_dir = "/".join(var_parts)          # e.g. tha/l2/fgm
...
def parse_themis_prefixes(text: str):
    """Parse THEMIS file prefixes, expanding 'th?' into tha/thb/thc/thd/the."""
    text = text.replace("；", ";").replace("，", ",")
    text = text.replace(",", ";").replace("\n", ";")
    prefixes = []
    for item in text.split(";"):
...
class HomeTab(ttk.Frame):
    """
    Landing page shown at startup. Lists available satellite downloaders
    with a brief description and a button to jump to each tab.
    """

    # Each entry: (tab_label, description)
    SATELLITES = [
        (
            "MMS",
    ...
    def _build_ui(self):
        ttk.Label(
            self,
            text="Satellite Data Downloader",
            font=("Segoe UI", 18, "bold"),
        ).pack(pady=(30, 6))

        ttk.Label(
            self,
            text="Select a satellite mission below to open its downloader.",
            font=("Segoe UI", 10),
            foreground="#555555",
        ).pack(pady=(0, 24))
    ...
    def _make_card(self, parent, tab_label: str, description: str):
        ...
        ttk.Label(card, text=description, justify="left", wraplength=700).pack(
            side="left", fill="x", expand=True
        )
        ttk.Button(
            card,
            text=f"Open {tab_label}",
            command=lambda t=tab_label: self._open_tab(t),
...
class MMSDownloaderTab(ttk.Frame):
    """MMS downloader UI, embedded as a notebook tab."""
    ...
    def _download_worker(self, prefixes, tint, dest, opts):
        ...
        try:
            ...
            for idx, prefix in enumerate(prefixes, start=1):
                ...
                root.after(0, lambda i=idx, n=total_prefixes, p=prefix: self.status_var.set(f"Scanning {i}/{n}: {p}"))
...
class THEMISDownloaderTab(ttk.Frame):
    """THEMIS downloader UI, embedded as a notebook tab."""
    ...
    def _download_worker(self, prefixes, tint, dest, opts):
        ...
        try:
            ...
            for idx, prefix in enumerate(prefixes, start=1):
                ...
                ok, skipped, fail = themis_download_bs4_mt(
                    file_prefix=prefix,
                    tint=tint,
                    destination_dir=dest,
                    opts=opts,
...
# Registry of satellite tabs: list of (tab_label, TabClass)
SATELLITE_TABS = [
